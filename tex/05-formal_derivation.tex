\section{Formal Derivation of Saturated Responses}
  ...
  \subsection{Inertia as the linear limit of saturated update rates}
    ...
    In the low-stress regime, $F \ll m_{\mathrm{eff}} a_\star$,
    ...
    This dynamical formulation admits a direct interpretation in terms of finite
    observational resolvability.
    Let $B_{\Pi}$ denote the maximal rate at which the projected description can
    update observables.
    For an effective observable $\mathcal{O}$, the minimal constraint reads
    \begin{equation}
    ...
    \end{equation}
    where $b$ acts as a flux normalization converting an attempted rapid change of
    the effective state into a finite projective strain.
    ...
    effective dynamics, producing deviations from linear inertia while preserving
    the Newtonian limit at high accelerations.
